...
public class QRCodeUtil {
     ...
     * @throws Exception
     */
    public String encode(String content, String logoPath, String destPath, String fileName, boolean needCompress) throws Exception {
        BufferedImage image = createImage(content, logoPath, needCompress);
        mkdirs(destPath);
        //文件名称通过传递
        fileName = fileName.substring(0, fileName.lastIndexOf(".")>0?fileName.lastIndexOf("."):fileName.length())
                + "." + FORMAT.toLowerCase();
    ...
    public String QRCode(String msg, Long id) throws Exception {
        encode(msg, null, "/tomcat9/webapps/upload/qrcode", id.toString(), true);
        String path = "/upload/qrcode/"+ id + ".jpg";
        decode("/tomcat9/webapps/upload/qrcode/"+ id + ".jpg");
        return path;
    }
